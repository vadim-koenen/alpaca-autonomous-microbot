...
## The tool
`scripts/audit_snapshot.sh` — read-only. No broker calls, no `.env`, no orders. Prints a digest and a final line `AUDIT_VERDICT=OK|WARN|CRITICAL` (exit code 0/1/2). It stores the last cumulative net in `reports/spot_checks/last_net.txt` to detect fresh bleeding between runs.
