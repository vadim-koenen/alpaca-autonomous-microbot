...
def test_current_downtrend_context_confirms_sit_out():
    dashboard = _dashboard()

    assert dashboard["verdict"] == "SIT_OUT_CONFIRMED"
...
def test_dashboard_includes_trend_advisory_without_trade_trigger():
    ...
    assert all(row["trend_advisory_action"] == "avoid" for row in dashboard["symbols"])
...
def test_btc_eth_only_and_sol_excluded():
    dashboard = _dashboard()
    ...
    assert dashboard["sizing"]["eligible_symbols"] == ["BTC/USD", "ETH/USD"]
    assert dashboard["sizing"]["excluded_symbols"] == ["SOL/USD"]


def test_candidate_context_still_has_no_trade_permission():
    dashboard = _dashboard(trend_source_json=FIXTURES / "uptrend_candidate_context.json")
    by_symbol = {row["symbol"]: row for row in dashboard["symbols"]}

    assert dashboard["verdict"] == "READY_TO_OBSERVE"
    assert by_symbol["BTC/USD"]["opportunity_verdict"] == "candidate"
    assert by_symbol["BTC/USD"]["allowed_strategies"] == ["momentum_breakout"]
    assert dashboard["trade_permission"] == "none"
    assert dashboard["live_order_actions_allowed"] is False
...
def test_missing_trend_source_blocks_symbol_evidence_safely(tmp_path):
    ...
    assert dashboard["verdict"] == "UNKNOWN"
