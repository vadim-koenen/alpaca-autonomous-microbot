...
# Class 1 diagnostic report; avoids live trading modules.
"""
...
def parse_iso_datetime(value: Any) -> Optional[datetime]:
    ...
    # Common journal format fallback.
    candidates.extend([
        text.replace(" ", "T"),
...
def extract_scalar(text: str, key: str) -> Optional[float]:
    # Accept lines like "max_single_trade_notional_usd: 1.00" anywhere in the file.
    match = re.search(rf"(?m)^\s*{re.escape(key)}\s*:\s*([-+]?\d+(?:\.\d+)?)\b", text)
    if not match:
        return None
    ...
    for line in lines:
        ...
        if section_match:
            ...
            if in_section and base_indent is not None and indent <= base_indent:
                in_section = False
        if in_section:
            match = re.match(rf"^\s*{re.escape(key)}\s*:\s*([-+]?\d+(?:\.\d+)?)\b", line)
            if match:
                return safe_float(match.group(1))
    return None
...
def load_config_snapshot(path: Path = DEFAULT_CONFIG_PATH) -> ConfigSnapshot:
    ...
    return ConfigSnapshot(
        ...
        controlled_max_single_trade_usd=extract_section_scalar(text, "controlled_exploration", "max_single_trade_notional_usd") or extract_scalar(text, "max_single_trade_notional_usd"),
        max_total_exploration_exposure_usd=extract_section_scalar(text, "controlled_exploration", "max_total_exploration_exposure_usd") or extract_scalar(text, "max_total_exploration_exposure_usd"),
        max_open_positions=safe_int(extract_section_scalar(text, "controlled_exploration", "max_open_positions") or extract_section_scalar(text, "global_risk", "max_open_positions")),
        maker_fee_pct=extract_section_scalar(text, "fees", "maker_fee_pct") or extract_scalar(text, "maker_fee_pct"),
        taker_fee_pct=extract_section_scalar(text, "fees", "taker_fee_pct") or extract_scalar(text, "taker_fee_pct"),
...
def normalize_trade_row(row: dict[str, str]) -> Optional[TradeRow]:
    ...
    if side not in {"buy", "sell"}:
        return None
    timestamp = parse_iso_datetime(first_value(row, ("timestamp", "timestamp_utc", "time", "created_at", "submitted_at", "filled_at", "exit_timestamp", "entry_timestamp")))
    quantity = first_float(row, ("quantity", "qty", "filled_qty", "base_size", "size", "amount"))
    price = first_float(row, ("price", "filled_avg_price", "average_price", "avg_price", "entry_price", "exit_price", "fill_price"))
    notional = first_float(row, ("notional", "notional_usd", "usd_notional", "trade_notional_usd", "filled_notional", "value_usd", "cost", "proceeds"))
...
def load_price_path_stats(path: Path = DEFAULT_PRICE_PATH) -> tuple[dict[tuple[str, str], PathStats], Optional[str]]:
    ...
    for (symbol, entry_ts), group in grouped.items():
        ...
        path = PathStats(
            ...
            latest_unrealized_pct=unrealized_values[-1] if unrealized_values else None,
            max_hold_minutes=max(hold_values) if hold_values else None,
            crossings={threshold: None for threshold in THRESHOLDS_PCT},
        )
...
def attach_price_paths(cycles: list[TradeCycle], paths: dict[tuple[str, str], PathStats]) -> None:
    for cycle in cycles:
        if cycle.entry_timestamp is None:
            continue
        candidates = [
            cycle.entry_timestamp.isoformat(),
            cycle.entry_timestamp.replace(tzinfo=None).isoformat(),
            cycle.entry_timestamp.strftime("%Y-%m-%dT%H:%M:%S%z"),
            cycle.entry_timestamp.strftime("%Y-%m-%d %H:%M:%S%z"),
        ]
        for candidate in candidates:
        ...
        if cycle.path_stats is not None:
            continue
        # Fuzzy fallback: same symbol and entry timestamp within two minutes.
...
def theoretical_dynamic_notional(config: ConfigSnapshot, observed_notional: Optional[float]) -> Optional[float]:
    ...
    # If observed live notional is higher than expected_starting_equity-derived value,
    # keep the conservative config-derived estimate and let final cap explain the result.
    return calc
...
def render_profitability_summary(cycles: list[TradeCycle]) -> list[str]:
    ...
    if not cycles:
        lines.extend(["Completed cycles: 0", ""])
        return lines
...
def render_symbol_summary(cycles: list[TradeCycle], config: ConfigSnapshot) -> list[str]:
    ...
    for symbol in sorted(by_symbol):
        ...
        gross = sum(c.gross_pnl or 0.0 for c in group)
        net = sum(c.net_pnl or 0.0 for c in group)
        ...
        if len(group) < 3:
            status = "inconclusive"
        elif net > 0 and break_even_crossings > 0:
            status = "promising"
        elif net < 0:
            ...
            status = "inconclusive"
        lines.extend([
            f"{symbol}: cycles={len(group)} gross={fmt_money(gross)} net={fmt_money(net)} avg_MFE={fmt_pct(mean(mfes) if mfes else None)} avg_MAE={fmt_pct(mean(maes) if maes else None)} best_MFE={fmt_pct(max(mfes) if mfes else None)} worst_MAE={fmt_pct(min(maes) if maes else None)} break_even_crossings={break_even_crossings} status={status}",
        ])
    lines.append("")
    return lines
